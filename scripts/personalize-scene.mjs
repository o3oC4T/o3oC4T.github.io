...
// Camera, geometry and animation logic are unchanged; colors/materials are themed.
import { readFile, writeFile, mkdir, copyFile } from 'node:fs/promises';
import { resolve } from 'node:path';
import { icons } from '../docs/content.js';
import { pastelScene, recolorCss, writeCursors } from './neon-pastel.mjs';
...
await writeFile(new URL('archive-scene.js', target), pastelScene(scene));
...
await writeFile(new URL('reference-layout.css', target), recolorCss(layout));
await writeFile(new URL('reference-mobile.css', target), recolorCss(mobile));
await writeCursors();
